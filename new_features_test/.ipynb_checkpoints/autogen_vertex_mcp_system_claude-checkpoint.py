...
TOOL_DEFINITIONS = [
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {"type": "string", "description": "The search query (e.g., 'CSV pipeline error handling')."},
                    "filters": {"type": "string", "description": "Optional filters (e.g., 'status:SUCCESS')."},
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "mission_id": {"type": "string", "description": "The unique ID of the mission."}
                },
                "required": ["mission_id"]
            }
        }
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {
                    "min_score": {"type": "number", "description": "Minimum score threshold (default 90.0)."}
                },
                "required": []
            }
        }
...
class SystemConfig:
    ...
    vertex_search_config: str = "projects/815755318672/locations/us/collections/default_collection/dataStores/external-memory-connector_1756845276280_gcs_store/servingConfigs/default_config"
    vertex_location: str = "us"
    
    # AutoGen config - możesz zmienić na Gemini jeśli masz setup
    autogen_config: Dict[str, Any] = field(default_factory=lambda: {
        "config_list": [{
    ...
    def analyze_patterns(self, min_score: float = 90.0) -> str:
        ...
        try:
            ...
            results_json = self.search_mission_memory(
                query="approved:true",
                filters=f"final_score >= {min_score}",
                top_k=20
            )
            ...
            patterns = {
                "optimization_rate": sum(1 for r in results if r.get("has_optimization")) / total,
                "rollback_rate": sum(1 for r in results if r.get("has_rollback")) / total,
...
class MissionPlanningTeam:
    ...
    def setup_agents(self):
        ...
        self.executor = UserProxyAgent(
            ...
            code_execution_config={"use_docker": False, "work_dir": "tool_execution_logs"},
        ...
        for msg in reversed(messages):
            if msg.get("name") == "GraphDesigner":
                ...
                try:
                    ...
                    import re
                    json_match = re.search(r'\{.*"entry_point".*"nodes".*"edges".*\}', 
                                         content, re.DOTALL)
                    if json_match:
        ...
        for msg in messages:
            if msg.get("name") == "MemoryAnalyst":
                ...
                if "improvement" in msg.get("content", "").lower():
                    context["applied_improvements"].append(
                        msg.get("content", "")[:200]
                    )
...
class MissionExecutor:
    ...
    async def execute_mission(self, mission_prompt: str) -> Dict[str, Any]:
        ...
        if result.get("learning_context", {}).get("analyzed_missions"):
            logging.info(f"Learned from missions: {result['learning_context']['analyzed_missions']}")
        
        logging.info(f"Plan created with {len(result['final_plan'].get('nodes', []))} nodes")
